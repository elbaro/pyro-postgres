...
class TestSyncAffectedRows:
    """Test sync affected_rows method."""

    def test_affected_rows_insert_single(self):
    ...
    def test_affected_rows_insert_multiple(self):
    ...
    def test_affected_rows_delete(self):
        ...
        conn.query_drop("INSERT INTO test_table (name, age) VALUES ('Alice', 30)")
        conn.query_drop("INSERT INTO test_table (name, age) VALUES ('Bob', 25)")
        conn.query_drop("DELETE FROM test_table WHERE age < 30")
        affected = conn.affected_rows()
        assert affected == 1
        cleanup_test_table_sync(conn)
        conn.close()

    def test_affected_rows_no_match(self):
        """Test affected_rows when no rows affected."""
        conn = Conn(get_test_db_url())
        setup_test_table_sync(conn)
        conn.query_drop("UPDATE test_table SET age = 100 WHERE id = 99999")
        affected = conn.affected_rows()
        assert affected == 0
        cleanup_test_table_sync(conn)
        conn.close()

    def test_affected_rows_select(self):
        ...
        conn.query("SELECT 1 UNION SELECT 2 UNION SELECT 3")
        affected = conn.affected_rows()
        assert affected == 3
        conn.close()
